...
public sealed class EventTests
{
    ...
    public void Create_starts_as_draft_with_all_seats_available_and_utc_start()
    {
        ...
        var @event = Event.Create(Organizer, " Jazz Night ", "", "Grand Hall", startsAt, 100, Now);
    ...
    [Fact]
    public void Create_records_the_organizer()
    {
        var @event = Event.Create("organizer-42", "Show", "", "Hall", Now.AddDays(1), 10, Now);

        @event.IsOrganizedBy("organizer-42").ShouldBeTrue();
        @event.IsOrganizedBy("someone-else").ShouldBeFalse();
        @event.IsOrganizedBy(null).ShouldBeFalse();
    }

    [Fact]
    public void Create_requires_an_organizer() =>
        Should.Throw<DomainException>(() =>
            Event.Create(" ", "Show", "", "Hall", Now.AddDays(1), 10, Now)
        );
    ...
    public void Create_rejects_out_of_range_capacity(int capacity) =>
        Should.Throw<DomainException>(() =>
            Event.Create(Organizer, "Show", "", "Hall", Now.AddDays(1), capacity, Now)
        );

    [Fact]
    public void Create_rejects_start_in_the_past() =>
        Should.Throw<DomainException>(() =>
            Event.Create(Organizer, "Show", "", "Hall", Now.AddMinutes(-1), 10, Now)
        );
    ...
    public void Reserve_holds_seats_until_expiry()
    {
        ...
        var reservation = @event.Reserve(Customer, "Fan@Example.com", 3, Now, Hold);

        @event.SeatsAvailable.ShouldBe(7);
        reservation.IsHeldBy(Customer).ShouldBeTrue();
        reservation.IsHeldBy("someone-else").ShouldBeFalse();
    ...
    {
        var draft = Event.Create(Organizer, "Show", "", "Hall", Now.AddDays(1), 10, Now);

        Should
            .Throw<DomainException>(() => draft.Reserve(Customer, "a@b.com", 1, Now, Hold))
    ...
    public void Reserve_rejects_more_seats_than_available()
    {
        ...
        @event.Reserve(Customer, "a@b.com", 2, Now, Hold);

        Should
            .Throw<DomainException>(() => @event.Reserve(Customer, "c@d.com", 2, Now, Hold))
    ...
    public void Reserve_reports_sold_out()
    {
        ...
        @event.Reserve(Customer, "a@b.com", 1, Now, Hold);

        Should
            .Throw<DomainException>(() => @event.Reserve(Customer, "c@d.com", 1, Now, Hold))
    ...
    public void Reserve_rejects_events_that_have_started()
    {
        ...
        Should.Throw<DomainException>(() =>
            @event.Reserve(Customer, "a@b.com", 1, Now.AddDays(8), Hold)
        );
    }
    ...
    public void CancelReservation_releases_seats()
    {
        ...
        var reservation = @event.Reserve(Customer, "a@b.com", 4, Now, Hold);
    ...
    {
        var @event = PublishedEvent(capacity: 5);
        var reservation = @event.Reserve(Customer, "a@b.com", 2, Now, Hold);
    ...
    {
        var @event = PublishedEvent();
        var reservation = @event.Reserve(Customer, "a@b.com", 1, Now, Hold);
    ...
    {
        var @event = PublishedEvent(capacity: 10);
        @event.Reserve(Customer, "a@b.com", 6, Now, Hold);
    ...
    {
        var @event = PublishedEvent();
        var pending = @event.Reserve(Customer, "a@b.com", 1, Now, Hold);
        var confirmed = @event.Reserve(Customer, "c@d.com", 2, Now, Hold);
        ...
        Should.Throw<DomainException>(() => @event.Reserve(Customer, "e@f.com", 1, Now, Hold));
    }
